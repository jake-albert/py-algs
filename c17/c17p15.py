...
# Since calls to word_made_of_others() on different words in the list do not
# share information with each other, a call that failed on "abcde" but did 
# verify that "abc" is made up of other words currently throws that 
# information away, so there is still some duplicated work on certain inputs.

# Would it be worth it to also maintain some global hash set that stores 
# shorter words verified before their turn? It would speed up performance on 
# the final two inputs but would slow performance slightly for other inputs 
